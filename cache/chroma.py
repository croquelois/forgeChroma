...
class IntegratedChromaTransformer2DModel(nn.Module):
    def __init__(self, in_channels: int, vec_in_dim: int, context_in_dim: int, hidden_size: int, mlp_ratio: float, num_heads: int, depth: int, depth_single_blocks: int, axes_dim: list[int], theta: int, qkv_bias: bool, guidance_out_dim: int, guidance_hidden_dim: int, guidance_n_layers: int):
        ...
        self.previous_modulated_input = [None,None] # TEA-CACHE
        self.teacache_COEFS = [4.98651651e+02, -2.83781631e+02,  5.58554382e+01, -3.82021401e+00, 2.64230861e-01] # TEA-CACHE
        self.accumulated_ratio = [1.0, 1.0] # MAG-CACHE
        self.magcache_AMPLI = 0.05 # MAG-CACHE
        self.magcache_SHARP = 0.85 # MAG-CACHE
        # END CACHE
    ...
    def setCache(self, cache_type, cache_thresh, cache_max, cache_retention):
    ...
    def inner_forward(self, img, img_ids, txt, txt_ids, timesteps, cond_or_uncond, step_pct):
        ...
        cache_idx = cond_or_uncond
        skip_forward = False
        self.accumulated_steps[cache_idx] += 1
        is_not_last_or_1st = step_pct > 0.0 and step_pct < 1.0
        if self.cache_type == "MAG-CACHE":
            cur_mag_ratio = 1-self.magcache_AMPLI*math.tan(self.magcache_SHARP*(step_pct-0.5)*math.pi)
            self.accumulated_ratio[cache_idx] = self.accumulated_ratio[cache_idx] * cur_mag_ratio
            if is_not_last_or_1st:
                cur_skip_err = np.abs(1 - self.accumulated_ratio[cache_idx])
                self.accumulated_err[cache_idx] += cur_skip_err
                
        if self.cache_type == "TEA-CACHE":
            (img_mod1, img_mod2) = mod_vectors_dict[f"double_blocks.0.img_mod.lin"]
            inp_modulated = self.double_blocks[0].img_norm1(img.clone())
            inp_modulated = (1 + img_mod1.scale) * inp_modulated + img_mod1.shift
            if is_not_last_or_1st:
                rescale_func = np.poly1d(self.teacache_COEFS)
                prev_mod_input = self.previous_modulated_input[cache_idx]
                cur_skip_err = rescale_func(((inp_modulated-prev_mod_input).abs().mean() / prev_mod_input.abs().mean()).cpu().item())
                self.accumulated_err[cache_idx] += cur_skip_err
            self.previous_modulated_input[cache_idx] = inp_modulated
            
        if self.cache_type != "NONE" and step_pct >= self.cache_retention and is_not_last_or_1st and self.accumulated_err[cache_idx] < self.cache_thresh and self.accumulated_steps[cache_idx] <= self.cache_max:
            skip_forward = True

        if skip_forward:
            return self.final_layer(img + self.residual_cache[cache_idx], mod_vectors_dict["final_layer.adaLN_modulation.1"])
            
        ori_img = img.clone() ## CACHE
        self.accumulated_err[cache_idx] = 0
        self.accumulated_steps[cache_idx] = 0
        self.accumulated_ratio[cache_idx] = 1.0
        ...
        del pe
        img = img[:, txt.shape[1]:, ...]
        res = img - ori_img ## CACHE
        self.residual_cache[cache_idx] = res ## CACHE
        img = self.final_layer(img, mod_vectors_dict["final_layer.adaLN_modulation.1"])
        return img

    def forward(self, x, timestep, context, **kwargs):
        cond_or_uncond = kwargs['transformer_options']['cond_or_uncond'][0]
        step_pct = kwargs['transformer_options']['step_pct']
        bs, c, h, w = x.shape
        ...
        out = self.inner_forward(img, img_ids, context, txt_ids, timestep, cond_or_uncond, step_pct)
